...
class NewSupplierGASUsage(serializers.Serializer):
    stading_charge = serializers.DecimalField(max_digits=6, decimal_places=4,required=False)
    standing_charge_uplift = serializers.DecimalField(max_digits=6, decimal_places=4,required=False)
    rate = serializers.DecimalField(max_digits=6, decimal_places=4,required=False)
    unit_rate_uplift = serializers.DecimalField(max_digits=6, decimal_places=4,required=False)
...
class NewSupplierELECTRICUsage(serializers.Serializer):
    ...
    feed_in_tariff = serializers.DecimalField(max_digits=6, decimal_places=4,required=False)
    annual_day_usage = serializers.IntegerField(required=False)
    day_rate = serializers.DecimalField(max_digits=6, decimal_places=4, required=False)
    night_rate = serializers.DecimalField(max_digits=6, decimal_places=4, required=False)
    annual_night_usage = serializers.IntegerField(required=False)
    evening_rate = serializers.DecimalField(max_digits=6, decimal_places=4, required=False)
...
class NewSupplyUsageRateSerialize(serializers.Serializer):
    gas_usage_details = NewSupplierGASUsage(required=False)
    electric_usage_details = NewSupplierELECTRICUsage(required=False)
...
class New_supplies_Serializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        elec_usage_rate_instance = instance.usage_rates.filter(usage_type="ELECTRIC").first()
        ...
        if electric_usage_rate and elec_usage_rate_instance:
            ...
            elec_usage_rate_instance.save(update_fields = list(electric_usage_rate.keys()))
    ...
    def to_representation(self, instance):
        ...
        gas_usages_rate_instance = instance.usage_rates.all().filter(usage_type="GAS").first()
        electric_usages_rate_instance = instance.usage_rates.all().filter(usage_type="ELECTRIC").first()
        # usage_rates = {}
        ...
        if electric_usages_rate_instance:
            data['electric_usage_rate'] = NewSupplierELECTRICUsage(electric_usages_rate_instance).data
        return data
